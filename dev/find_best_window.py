...
SAMPLE_FPS = 10
CLIP_DUR = 6
SCAN_BEFORE = 8   # seconds before the topic timestamp to start scanning
SCAN_AFTER = 30   # seconds after to scan
SCAN_STEP = 1.0   # seconds between motion-probe samples

# (output_name, topic_timestamp_sec, crop_width_of_1920)
TOPICS = [
    ("job_order",              57,  1672),
    ("profession_specialty",  177,  1672),
    ("keywords_andor",        297,  1672),
    ("realtime_boards",       657,  1672),
    ("applicants_tab",       1137,  1672),
    ("divamatch",             1427, 1672),
    ("availability",          1607, 1672),
    ("preferred_locations",   2682, 1672),
    ("licenses_certs_tabs",   3447, 1672),
    ("candidate_wallet",      2037, 1672),
    ("email_optout",          1787, 1672),
    ("live_interview_email",  2557, 1920),
    ("notes",                 3047, 1672),
    ("internal_submission",  4617,  1672),
    ("post_boards",           3817, 1672),
]
...
for name, ts, crop_w in TOPICS:
    ...
    for t in probe_times:
        ...
        if f is not None:
            frames.append((t, f))
    if len(frames) < 3:
        print(f"{name}: not enough frames to scan, using original timestamp")
        best_start = ts
    else:
        diffs = []
        for i in range(1, len(frames)):
            ...
            diffs.append(d)
        # sliding window over CLIP_DUR seconds worth of diff samples, find max total motion
    ...
    # Now extract the actual high-res clip from best_start
